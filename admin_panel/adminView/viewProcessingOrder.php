<div id="ordersBtn">
    <p class="section-title">Processing Orders</p>
    <div class="scrollable-table">
        <table class="table" style="width: 80vw;">
            ...
            $sql = "SELECT * from orders, mode_of_payment,users WHERE orders.payment_method_id = mode_of_payment.payment_method_id AND orders.order_status = 1 AND orders.user_id = users.user_id";
            ...
            ?>
                    <tr>
                        ...
                        ?>
                            <td><button class="btn btn-danger" onclick="ChangeToShipped('<?= $row['order_id'] ?>')">Pending </button></td>
                        <?php

                        } elseif ($row["order_status"] == 1) {
                        ?>
                            <td><button class="btn btn-warning" onclick="ChangeToShipped('<?= $row['order_id'] ?>')">Processing</button></td>
                        <?php
                        } elseif ($row["order_status"] == 2) {
                        ?>
                            <td><button class="btn btn-info" onclick="ChangeToShipped('<?= $row['order_id'] ?>')">Shipped</button></td>
                        <?php
                        } elseif ($row["order_status"] == 3) {
                        ?>
                            <td><button class="btn btn-success" onclick="ChangeToShipped('<?= $row['order_id'] ?>')">Delivered</button></td>
                        <?php
                        } elseif ($row["order_status"] == 4) {
                        ?>
                            <td><button class="btn btn-success" onclick="ChangeToShipped('<?= $row['order_id'] ?>')">Cancelled</button></td>
                        <?php
                        }
                        if ($row["pay_status"] == 0) {
                        ?>
